...
export async function runLLMSession(
  ...
  // Single asset per session — use config.assetClass directly
  ...
  for (let round = 1; round <= nRounds; round++) {
    ...
    for (let period = 1; period <= nPeriods; period++) {
      ...
      const decisionPromises = agents.map(agent => {
        const { system, user } = buildPrompt(plan, agent, baseCtx, config);
      ...
      for (let tick = 0; tick < ticksPerPeriod; tick++) {
        ...
        // Proxy exposes { id, cash, shares } — match reads/writes shares as a plain number
      ...
      // Period-end: compute VWAP, update beliefs + trust
