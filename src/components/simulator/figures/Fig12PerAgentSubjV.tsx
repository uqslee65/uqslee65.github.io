...
export function Fig12PerAgentSubjV() {
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number) => {
    ...
    periods.forEach((p, i) => {
      ...
      ctx.beginPath();
      ctx.moveTo(x1, yScale(p.fv));
      ctx.lineTo(x2, yScale(p.fv));
      ctx.stroke();
    });
    ctx.setLineDash([]);

    // Per-agent belief lines (fan chart stub: always use lines for current default N)
